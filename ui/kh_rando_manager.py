...
""" + "\n".join(f"• {os.path.basename(f)}" for f in files_to_convert[:5])
    ...
    def fix_duplicates_in_kh_rando(self):
        """Remove duplicate files from KH Rando folder (keeping originals in library)"""
        if not self.config.kh_rando_folder:
            show_themed_message(self.main_window, QMessageBox.Warning, "No KH Rando Folder", "Please select a KH Rando folder first.")
            return
        
        # Find files that are duplicates (orange colored in library)
        duplicate_files = []
        for i in range(self.file_list.count()):
            item = self.file_list.item(i)
            file_path = item.data(Qt.UserRole)
            if file_path and os.path.exists(file_path):
                # Check if it's orange (duplicate) - using foreground color
                if item.foreground().color().name() == '#ffa500':  # Orange text
                    base_name = os.path.splitext(os.path.basename(file_path))[0].lower()
                    # Find the duplicate in KH Rando folder by comparing base names
                    for category in self.kh_rando_exporter.MUSIC_CATEGORIES.keys():
                        category_path = os.path.join(self.config.kh_rando_folder, category)
                        if os.path.exists(category_path):
                            for file in os.listdir(category_path):
                                kh_base_name = os.path.splitext(file)[0].lower()
                                if base_name == kh_base_name:
                                    duplicate_path = os.path.join(category_path, file)
                                    duplicate_files.append((duplicate_path, category, file))
        
        if not duplicate_files:
            show_themed_message(self.main_window, QMessageBox.Information, "No Duplicates", "No duplicate files found in KH Rando folder.")
            return
        
        # Show confirmation
        msg = f"Remove {len(duplicate_files)} duplicate files from KH Rando folder?\n\n"
        msg += "This will delete files from KH Rando but keep the originals in your library.\n\n"
        msg += "Duplicates to remove:\n" + "\n".join(f"• {cat}/{file}" for _, cat, file in duplicate_files[:10])
        if len(duplicate_files) > 10:
            msg += f"\n• ... and {len(duplicate_files) - 10} more"
        
        reply = show_themed_message(self.main_window, QMessageBox.Question, "Remove Duplicates", msg,
                                   QMessageBox.Yes | QMessageBox.No,
                                   QMessageBox.No)
        if reply != QMessageBox.Yes:
            return
        ...
        removed_count = 0
        for duplicate_path, category, filename in duplicate_files:
            try:
        ...
        show_themed_message(self.main_window, QMessageBox.Information, "Duplicates Removed", f"Removed {removed_count} duplicate files from KH Rando folder.")
